...
# WebRTC — live camera on Streamlit Cloud
...
_state = {
    ...
    "feature_store": deque(maxlen=MAX_HISTORY),
    ...
    "is_training":   False,
    "weights":       {"face": 50.0, "tonal": 25.0, "text": 25.0},
    "frame_count":   0,
    "fps":           0.0,
    "_fps_t":        time.time(),
    "_fps_n":        0,
...
def train_all_models():
    ...
    try:
        ...
        if len(current_X) < MIN_TRAIN: return
...
def _push_frame_to_state(face_e):
    """Called every WebRTC frame — updates all derived state, triggers training."""
    with _lock:
    ...
    with _lock:
        ...
        sess = _state["session"]
        sess["timestamps"].append(time.time())
        _state["feature_store"].append(build_fv(face_e, tonal_e, text_e))
...
class FaceProcessor(VideoProcessorBase):
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        with _lock:
            ...
            xe  = _state["text_e"]
            fne = _state["final_e"]
            ml  = dict(_state["ml_preds"])

        annotated = _draw_overlay(img.copy(), fe, te, xe, fne, ml)
        return av.VideoFrame.from_ndarray(annotated, format="bgr24")

def _draw_overlay(frame, face_e, tonal_e, text_e, final_e, ml_preds):
    h, w = frame.shape[:2]
    ov = frame.copy()
    cv2.rectangle(ov, (0,0), (380,200), (5,10,16), -1)
    cv2.addWeighted(ov, 0.7, frame, 0.3, 0, frame)

    def ec(e):
        m = {"happy":(45,220,80),"sad":(100,140,255),"angry":(60,60,255),
             "fear":(200,50,200),"disgust":(200,180,30),"surprise":(0,184,255),
             "neutral":(150,150,150)}
        return m.get(e,(200,200,200))

    font = cv2.FONT_HERSHEY_SIMPLEX
    for txt, e, y in [(f"FACE  {face_e.upper()}",face_e,30),
                      (f"VOICE {tonal_e.upper()}",tonal_e,58),
                      (f"TEXT  {text_e.upper()}",text_e,86)]:
        cv2.putText(frame, txt, (12,y), font, 0.52, ec(e), 2, cv2.LINE_AA)
    cv2.line(frame,(10,98),(370,98),(40,40,40),1)
    cv2.putText(frame, f"FUSED {final_e.upper()}", (12,122), font, 0.65, ec(final_e), 2, cv2.LINE_AA)
    cv2.line(frame,(10,132),(370,132),(40,40,40),1)
    y_off = 152
    for nm, pred in list(ml_preds.items())[:4]:
        c = ec(pred) if pred!="—" else (60,60,60)
        cv2.putText(frame, f"{nm[:9]:<9} {pred.upper() if pred!='—' else '...'}",
                    (12,y_off), font, 0.38, c, 1, cv2.LINE_AA)
        y_off += 20
    cv2.putText(frame, f"{_state.get('fps',0):.0f} fps",
                (w-80,20), font, 0.45, (0,220,120), 1, cv2.LINE_AA)
    seg_w = w//len(EMOTIONS)
    for i,e in enumerate(EMOTIONS):
        cv2.rectangle(frame,(i*seg_w,h-6),((i+1)*seg_w,h),ec(e),-1)
    return frame
...
def _base_layout(title="", height=260):
    return dict(
        ...
        height=height, margin=dict(l=40,r=15,t=40,b=30),
...
def chart_emotion_frequency(sess):
    final_str = list(sess["final_str"])
    counts = [final_str.count(e) for e in EMOTIONS]
    fig = go.Figure(go.Bar(
        x=[e.upper() for e in EMOTIONS], y=counts,
        marker=dict(color=[EMOTION_COLORS.get(e,"#888") for e in EMOTIONS],
                    line=dict(color="rgba(0,0,0,0.4)",width=1)),
        text=counts, textposition="outside", textfont=dict(size=9,color="#aaa")))
...
def chart_model_accuracy(ml_trained, feature_store, session):
    ...
    if len(feature_store) >= MIN_TRAIN:
        ...
        y = np.array(list(session["final_seq"]))
        if len(np.unique(y)) >= 2:
            with _ml_lock:
                for nm, pipe in ML_MODELS.items():
                    if ml_trained.get(nm):
                        try:
                            accs.append(round(accuracy_score(y,pipe.predict(X))*100,1))
                            names.append(nm)
                        except: pass
    if not names:
        fig = go.Figure()
...
def chart_cv_scores(ml_cv_scores):
    ...
    if not trained_cv:
        ...
        return fig
    fig = go.Figure()
    cls = [ACCENT,ACCENT2,"#FF6B9D","#FFB800","#2DFF7A","#FF4040"]
    for i,(nm,cv_vals) in enumerate(trained_cv.items()):
        mv,sv = np.mean(cv_vals)*100, np.std(cv_vals)*100
        fig.add_trace(go.Bar(name=nm,x=[nm],y=[mv],
            error_y=dict(type="data",array=[sv],visible=True,color="#fff",thickness=1.5),
            marker=dict(color=cls[i%len(cls)],opacity=0.85),
            text=f"{mv:.1f}±{sv:.1f}%",textposition="outside",
...
def chart_confusion_matrix(model_name, feature_store, session, ml_trained, colorscale="YlOrRd"):
    ...
    fig = go.Figure(go.Heatmap(z=cm_n,x=short,y=short,colorscale=colorscale,
...
def chart_model_agreement(feature_store, ml_trained):
    ...
    with _ml_lock:
        ...
        for nm in trained_names:
            try:    pd_[nm]=ML_MODELS[nm].predict(X_sub)
            except: pd_[nm]=np.full(len(X_sub),4)
    for i,n1 in enumerate(trained_names):
        for j,n2 in enumerate(trained_names):
            agree[i,j]=np.mean(pd_[n1]==pd_[n2])
    sn = [nm[:7] for nm in trained_names]
    tv = [[f"{agree[i][j]:.2f}" for j in range(len(trained_names))] for i in range(len(trained_names))]
    fig = go.Figure(go.Heatmap(z=agree,x=sn,y=sn,colorscale="Greens",
        text=tv,texttemplate="%{text}",textfont=dict(size=9),showscale=False,zmin=0,zmax=1))
    fig.update_layout(**_base_layout("◈ MODEL AGREEMENT HEATMAP",260))
    fig.update_yaxes(autorange="reversed")
    return fig
...
def chart_rf_feature_importance(ml_trained):
    if not ml_trained.get("RandForest"):
        ...
        return fig
    with _ml_lock:
        try:    imp=ML_MODELS["RandForest"].named_steps["clf"].feature_importances_
        except: imp=np.zeros(6)
    feat_names = ["Face","Tonal","Text","F-conf","T-conf","X-conf"]
    clrs = [EMOTION_COLORS.get(e,"#888") for e in
            ["happy","sad","surprise","fear","angry","disgust"]]
    fig = go.Figure(go.Bar(x=imp,y=feat_names,orientation="h",
        marker=dict(color=clrs,line=dict(color="rgba(0,0,0,0.3)",width=1)),
...
def chart_emotion_donut(sess):
    final_str = list(sess["final_str"])
    counts = [final_str.count(e) for e in EMOTIONS]
    if sum(counts)==0: counts=[1]*N_EMOTIONS
    fig = go.Figure(go.Pie(
        labels=[e.upper() for e in EMOTIONS], values=counts, hole=0.6,
        marker=dict(colors=[EMOTION_COLORS.get(e,"#888") for e in EMOTIONS],
                    line=dict(color=CARD_BG,width=2)),
        textinfo="label+percent", textfont=dict(size=9), showlegend=False))
    fig.update_layout(
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        height=220, margin=dict(l=10,r=10,t=30,b=10),
        title=dict(text="◈ DISTRIBUTION",
                   font=dict(color=ACCENT,size=11,family="'Courier New',monospace"),
                   x=0.04,xanchor="left"),
...
<style>
  ...
  html,body,[class*="css"]{{
    background-color:{BG}!important;color:#c0c0c0!important;
    font-family:'Share Tech Mono',monospace!important;}}
  .block-container{{padding:1rem 1.5rem!important;max-width:100%!important;}}
  section[data-testid="stSidebar"]{{background:#080f15!important;border-right:1px solid rgba(0,229,255,0.15);}}
  section[data-testid="stSidebar"]*{{color:#9ab!important;font-family:'Share Tech Mono',monospace!important;}}
  [data-testid="metric-container"]{{background:{CARD_BG}!important;border:1px solid rgba(0,229,255,0.18)!important;border-radius:4px!important;padding:0.5rem 0.8rem!important;}}
  ...
  .dash-header{{font-family:'Orbitron',monospace;font-size:1.55rem;font-weight:900;color:{ACCENT};letter-spacing:0.12em;text-shadow:0 0 18px rgba(0,229,255,0.5);line-height:1.1;}}
  .dash-sub{{font-family:'Share Tech Mono',monospace;font-size:0.72rem;color:#345;letter-spacing:0.2em;}}
  .info-card{{background:{CARD_BG};border:1px solid rgba(0,229,255,0.12);border-radius:4px;padding:0.8rem 1rem;margin-bottom:0.5rem;}}
  .live-dot{{display:inline-block;width:9px;height:9px;border-radius:50%;background:#2DFF7A;box-shadow:0 0 8px #2DFF7A;animation:blink 1.2s ease infinite;margin-right:6px;}}
  @keyframes blink{{0%,100%{{opacity:1}}50%{{opacity:0.2}}}}
  .transcript-box{{background:rgba(0,229,255,0.04);border:1px solid rgba(0,229,255,0.15);border-left:3px solid {ACCENT};padding:0.5rem 0.8rem;font-size:0.78rem;color:#8ab;font-style:italic;border-radius:0 4px 4px 0;min-height:40px;}}
  .scan-line{{border:none;border-top:1px solid rgba(0,229,255,0.12);margin:0.6rem 0;}}
  .status-bar{{display:flex;gap:1rem;align-items:center;padding:0.4rem 0.8rem;background:#080f15;border-radius:3px;border:1px solid rgba(0,229,255,0.08);font-size:0.7rem;color:#456;margin-bottom:0.6rem;}}
  .status-ok{{color:#2DFF7A;}}.status-warn{{color:#FFB800;}}
...
with st.sidebar:
    ...
    fw_face  = st.slider("Face weight",  0, 100, 50, step=5)
    fw_tonal = st.slider("Voice weight", 0, 100, 25, step=5)
    fw_text  = st.slider("Text weight",  0, 100, 25, step=5)

    st.markdown("**Session**")
    refresh_ms = st.slider("Refresh rate (ms)", 200, 2000, 500, step=100)

    st.markdown('<hr class="scan-line">', unsafe_allow_html=True)
    _ns = len(_state["session"]["final_seq"])
    _nt = sum(_state["ml_trained"].values())
    st.markdown(f"""
    <div class="info-card" style="font-size:0.72rem">
      <span style="color:#567">SAMPLES</span>
      <span style="color:{ACCENT};float:right;font-family:'Orbitron',monospace">{_ns}</span><br>
      <span style="color:#567">MODELS TRAINED</span>
      <span style="color:{'#2DFF7A' if _nt==6 else '#FFB800'};float:right">{_nt}/6</span><br>
      <span style="color:#567">MIN FOR TRAIN</span>
      <span style="color:#456;float:right">{MIN_TRAIN}</span>
    </div>""", unsafe_allow_html=True)

    st.markdown("""<div style="font-size:0.65rem;color:#345;line-height:1.7">
    ◈ DeepFace facial emotion (WebRTC)<br>
...
<div class="status-bar">
  <span>WEBCAM <span class="status-ok">▶ WEBRTC</span></span>
  ...
  <span style="margin-left:auto;color:#234">FUSION: FACE {int(fw_face)}% · VOICE {int(fw_tonal)}% · TEXT {int(fw_text)}%</span>
</div>""", unsafe_allow_html=True)

# ─────────────────────────────────────────────────────────────────
# ROW 1: LIVE FEED + EMOTIONS + ML PREDICTIONS
# ─────────────────────────────────────────────────────────────────
cam_col, emo_col, ml_col = st.columns([2, 1.4, 1.6])

with cam_col:
    st.markdown('<div style="font-size:0.7rem;color:#345;letter-spacing:0.15em;margin-bottom:4px">◈ LIVE FEED (WebRTC)</div>', unsafe_allow_html=True)
    webrtc_streamer(
        key="sentiment-cam",
...
with emo_col:
    ...
    def emo_card(label, emotion):
        c = EMOTION_COLORS.get(emotion, "#888")
        return (f'<div style="background:{CARD_BG};border:1px solid {c}33;border-left:3px solid {c};'
                f'border-radius:3px;padding:0.45rem 0.7rem;margin-bottom:0.35rem;display:flex;'
                f'justify-content:space-between;align-items:center">'
                f'<span style="font-size:0.65rem;color:#456;letter-spacing:0.1em">{label}</span>'
                f'<span style="color:{c};font-family:\'Orbitron\',monospace;font-size:0.8rem;font-weight:700">◈ {emotion.upper()}</span></div>')

    st.markdown(emo_card("FACE EXPRESSION", face_e) +
    ...
    st.markdown('<div style="font-size:0.65rem;color:#345;margin-top:0.5rem;margin-bottom:2px">◈ MIC INPUT</div>', unsafe_allow_html=True)
    audio_bytes = st.audio_input("Record voice snippet", key="mic_input")
    if audio_bytes is not None:
        try:
            y_audio, sr_audio = librosa.load(io.BytesIO(audio_bytes.read()), sr=16000, mono=True)
            new_tonal = extract_voice_tonality(y_audio, sr_audio)
            with _lock:
                _state["tonal_e"] = new_tonal
        except Exception as exc:
            st.caption(f"Audio err: {exc}")

    # ── Text input → RoBERTa ───────────────────────────────────
    st.markdown('<div style="font-size:0.65rem;color:#345;margin-top:0.3rem;margin-bottom:2px">◈ TEXT SENTIMENT</div>', unsafe_allow_html=True)
    user_text = st.text_input("", key="txt_in", placeholder="Type to analyse sentiment…")
    if user_text:
        new_text_e = analyze_text(user_text, text_pipeline)
        with _lock:
            _state["text_e"]     = new_text_e
            _state["transcript"] = user_text

    short_t = transcript[-90:] if len(transcript)>90 else transcript
    st.markdown(f'<div class="transcript-box">"{short_t or "— listening —"}"</div>',
                unsafe_allow_html=True)
...
with ml_col:
    ...
    for i, (nm, pred) in enumerate(ml_preds.items()):
        ...
        <div style="background:{CARD_BG};border:1px solid rgba(255,255,255,0.05);
          ...
          <span style="color:{mc};font-size:0.68rem">{nm}</span>
          <span style="color:{tag_c};font-size:0.58rem">{tag}</span>
          <span style="color:{c};font-family:'Orbitron',monospace;font-size:0.75rem;font-weight:700">{pred.upper()}</span>
        </div>""", unsafe_allow_html=True)

    st.plotly_chart(chart_emotion_donut(session), use_container_width=True,
                    config={"displayModeBar": False})

# ─────────────────────────────────────────────────────────────────
# ROW 2: TIMELINE + FREQUENCY
# ─────────────────────────────────────────────────────────────────
st.markdown('<hr class="scan-line">', unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────
st.markdown('<hr class="scan-line">', unsafe_allow_html=True)
ac, cv, fi = st.columns(3)
with ac: st.plotly_chart(chart_model_accuracy(ml_trained, feature_store, session),   use_container_width=True, config={"displayModeBar":False})
with cv: st.plotly_chart(chart_cv_scores(ml_cv_scores),                               use_container_width=True, config={"displayModeBar":False})
with fi: st.plotly_chart(chart_rf_feature_importance(ml_trained),                     use_container_width=True, config={"displayModeBar":False})

# ─────────────────────────────────────────────────────────────────
# ROW 4: CONFUSION + AGREEMENT
# ─────────────────────────────────────────────────────────────────
st.markdown('<hr class="scan-line">', unsafe_allow_html=True)
c1, c2, c3, c4 = st.columns(4)
with c1: st.plotly_chart(chart_confusion_matrix("LogReg",     feature_store, session, ml_trained, "YlOrRd"),  use_container_width=True, config={"displayModeBar":False})
with c2: st.plotly_chart(chart_confusion_matrix("SVM",        feature_store, session, ml_trained, "Blues"),   use_container_width=True, config={"displayModeBar":False})
with c3: st.plotly_chart(chart_confusion_matrix("RandForest", feature_store, session, ml_trained, "Greens"),  use_container_width=True, config={"displayModeBar":False})
with c4: st.plotly_chart(chart_model_agreement(feature_store, ml_trained),                                     use_container_width=True, config={"displayModeBar":False})
...
<div style="margin-top:1.5rem;padding:0.6rem 1rem;background:#050a0e;
  ...
  <span>⬡ MULTIMODAL SENTIMENT · DeepFace + Librosa + RoBERTa · WebRTC</span>
